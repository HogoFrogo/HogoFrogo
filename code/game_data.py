xyz = 420;
level_0 = {
		'terrain': '../levels/0/level_0_terrain.csv',
		'coins':'../levels/0/level_0_coins.csv',
		'fg palms':'../levels/0/level_0_fg_palms.csv',
		'bg palms':'../levels/0/level_0_bg_palms.csv',
		'crates': '../levels/0/level_0_crates.csv',
		'enemies':'../levels/0/level_0_enemies.csv',
		'constraints':'../levels/0/level_0_constraints.csv',
		'player': '../levels/0/level_0_player.csv',
		'grass': '../levels/0/level_0_grass.csv',
		'houses':'../levels/0/level_0_houses.csv',
		'node_pos': (110,100),
		'unlock': 1,
		'node_graphics': '../graphics/levels/0/overworld',
		'level_bg_music': '../audio/feed_the_machine.mp3',
		'flies': 2,
		'fireflies': 0,
		'dragonflies': 0,
		'wasps': 0,
		'flies_hard': 1,
		'dragonflies_hard': 1,
		'wasps_hard': 2,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'mosquitos_hard': 0,
		'fireflies_hard': 0,
		'goal_image': '../graphics/misc/sad_frog.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'First steps on new land',
		'level_img': '../graphics/levels/0/level_image.png',
		'biome': 'lawn_day'}
level_1 = {
		'terrain': '../levels/1/level_1_terrain.csv',
		'coins':'../levels/1/level_1_coins.csv',
		'fg palms':'../levels/1/level_1_fg_palms.csv',
		'bg palms':'../levels/1/level_1_bg_palms.csv',
		'crates': '../levels/1/level_1_crates.csv',
		'enemies':'../levels/1/level_1_enemies.csv',
		'constraints':'../levels/1/level_1_constraints.csv',
		'player': '../levels/1/level_1_player.csv',
		'grass': '../levels/1/level_1_grass.csv',
		'houses':'../levels/1/level_1_houses.csv',
		'node_pos': (300,270),
		'node_graphics': '../graphics/levels/1/overworld',
		'unlock': 2,
		'level_bg_music': '../audio/magnetic_b-ing.mp3',
		'flies': 20,
		'fireflies': 0,
		'dragonflies': 1,
		'wasps': 0,
		'flies_hard': 20,
		'dragonflies_hard': 1,
		'wasps_hard': 4,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/misc/sandwich.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'Saving the gourmet sandwich',
		'level_img': '../graphics/levels/1/level_image.png',
		'biome': 'lawn_day'}
level_2 = {
		'terrain': '../levels/2/level_2_terrain.csv',
		'coins':'../levels/2/level_2_coins.csv',
		'fg palms':'../levels/2/level_2_fg_palms.csv',
		'bg palms':'../levels/2/level_2_bg_palms.csv',
		'crates': '../levels/2/level_2_crates.csv',
		'enemies':'../levels/2/level_2_enemies.csv',
		'constraints':'../levels/2/level_2_constraints.csv',
		'player': '../levels/2/level_2_player.csv',
		'grass': '../levels/2/level_2_grass.csv',
		'houses':'../levels/2/level_2_houses.csv',
		'node_pos': (350,400),
		'node_graphics': '../graphics/levels/2/overworld',
		'unlock': 3,
		'level_bg_music': '../audio/no_limit.mp3',
		'flies': 10,
		'fireflies': 0,
		'dragonflies': 3,
		'wasps': 5, 
		'flies_hard': 33,
		'dragonflies_hard': 30,
		'wasps_hard': 17,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'sandwich',
		'level_name': 'On the wave home',
		'level_img': '../graphics/levels/2/level_image.png',
		'biome': 'lawn_day'}
level_3 = {
		'terrain': '../levels/3/level_3_terrain.csv',
		'coins':'../levels/3/level_3_coins.csv',
		'fg palms':'../levels/3/level_3_fg_palms.csv',
		'bg palms':'../levels/3/level_3_bg_palms.csv',
		'crates': '../levels/3/level_3_crates.csv',
		'enemies':'../levels/3/level_3_enemies.csv',
		'constraints':'../levels/3/level_3_constraints.csv',
		'player': '../levels/3/level_3_player.csv',
		'grass': '../levels/3/level_3_grass.csv',
		'houses':'../levels/3/level_3_houses.csv',
		'node_pos': (430,610),
		'node_graphics': '../graphics/levels/3/overworld',
		'unlock': 4,
		'level_bg_music': '../audio/vekna_darkware.mp3',
		'flies': 5,
		'fireflies': 10,
		'dragonflies': 1,
		'wasps': 0,
		'flies_hard': 10,
		'dragonflies_hard': 2,
		'wasps_hard': 1,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 5,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'In the footsteps of fireflies',
		'level_img': '../graphics/levels/3/level_image.png',
		'biome': 'lawn_night'}
level_4 = {
		'terrain': '../levels/4/level_4_terrain.csv',
		'coins':'../levels/4/level_4_coins.csv',
		'fg palms':'../levels/4/level_4_fg_palms.csv',
		'bg palms':'../levels/4/level_4_bg_palms.csv',
		'crates': '../levels/4/level_4_crates.csv',
		'enemies':'../levels/4/level_4_enemies.csv',
		'constraints':'../levels/4/level_4_constraints.csv',
		'player': '../levels/4/level_4_player.csv',
		'grass': '../levels/4/level_4_grass.csv',
		'houses':'../levels/4/level_4_houses.csv',
		'node_pos': (610,350),
		'node_graphics': '../graphics/levels/4/overworld',
		'unlock': 5,
		'level_bg_music': '../audio/level_music.wav',
		'flies': 10,
		'fireflies': 0,
		'dragonflies': 2,
		'wasps': 0,
		'flies_hard': 20,
		'dragonflies_hard': 10,
		'wasps_hard': 10,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'Froggy Style',
		'level_img': '../graphics/levels/4/level_image.png',
		'biome': 'lawn_day'}
level_shop = {
		'terrain': '../levels/shop/shop_terrain.csv',
		'coins':'../levels/shop/shop_coins.csv',
		'fg palms':'../levels/shop/shop_fg_palms.csv',
		'bg palms':'../levels/shop/shop_bg_palms.csv',
		'crates': '../levels/shop/shop_crates.csv',
		'enemies':'../levels/shop/shop_enemies.csv',
		'constraints':'../levels/shop/shop_constraints.csv',
		'player': '../levels/shop/shop_player.csv',
		'grass': '../levels/shop/shop_grass.csv',
		'houses':'../levels/shop/shop_houses.csv',
		'node_pos': (880,110),
		#'node_pos': (1050,400),
		'node_graphics': '../graphics/levels/shop/overworld',
		'unlock': 5,
		'level_bg_music': '../audio/magnetic_b-ing.mp3',
		'flies': 10,
		'fireflies': 0,
		'dragonflies': 2,
		'wasps': 0,
		'flies_hard': 20,
		'dragonflies_hard': 10,
		'wasps_hard': 10,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'Shopping Swamp',
		'level_img': '../graphics/levels/shop/level_image.png',
		'biome': 'lawn_day'}
level_5 = {
		'terrain': '../levels/4/level_4_terrain.csv',
		'coins':'../levels/4/level_4_coins.csv',
		'fg palms':'../levels/4/level_4_fg_palms.csv',
		'bg palms':'../levels/4/level_4_bg_palms.csv',
		'crates': '../levels/4/level_4_crates.csv',
		'enemies':'../levels/4/level_4_enemies.csv',
		'constraints':'../levels/4/level_4_constraints.csv',
		'player': '../levels/4/level_4_player.csv',
		'grass': '../levels/4/level_4_grass.csv',
		'houses':'../levels/4/level_4_houses.csv',
		'node_pos': (930,300),
		'node_graphics':'../graphics/overworld/5',
		'unlock': 5,
		'level_bg_music': '../audio/bolero_ranchero.mp3',
		'flies': 10,
		'fireflies': 0,
		'dragonflies': 2,
		'wasps': 0,
		'flies_hard': 20,
		'dragonflies_hard': 10,
		'wasps_hard': 10,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 8,
		'mosquitos_hard': 12,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'Like a flower without water',
		'level_img': '../graphics/levels/5/level_image.png',
		'biome': 'desert'}
level_6 = {
		'terrain': '../levels/2/level_2_terrain.csv',
		'coins':'../levels/2/level_2_coins.csv',
		'fg palms':'../levels/2/level_2_fg_palms.csv',
		'bg palms':'../levels/2/level_2_bg_palms.csv',
		'crates': '../levels/2/level_2_crates.csv',
		'enemies':'../levels/2/level_2_enemies.csv',
		'constraints':'../levels/2/level_2_constraints.csv',
		'player': '../levels/2/level_2_player.csv',
		'grass': '../levels/2/level_2_grass.csv',
		'houses':'../levels/2/level_2_houses.csv',
		'node_pos': (1050,560),
		'node_graphics':'../graphics/overworld/5',
		'unlock': 5,
		'level_bg_music': '../audio/magnetic_b-ing.mp3',
		'flies': 10,
		'fireflies': 0,
		'dragonflies': 2,
		'wasps': 0,
		'flies_hard': 20,
		'dragonflies_hard': 10,
		'wasps_hard': 10,
		'parachute_frog': 0,
		'parachute_frog_hard': 0,
		'mosquitos': 0,
		'mosquitos_hard': 0,
		'mosquitos_sombreros': xyz,
		'mosquitos_sombreros_hard': xyz,
		'fireflies_hard': 0,
		'goal_image': '../graphics/character/hat.png',
		'boss': 'none',
		'player_image': 'default',
		'level_name': 'Finally back',
		'level_img': '../graphics/levels/6/level_image.png',
		'biome': 'lawn_day'}

levels = {
	0: level_0,
	1: level_1,
	2: level_2,
	3: level_3,
	4: level_4,
	5: level_shop,
	6: level_5,
	7: level_6}